...
struct Order {
    static void apply(DTRes *& res, const DTRes * arg, size_t * colIdxs, size_t numKeyCols, bool * ascending, DCTX(ctx), std::vector<std::pair<size_t, size_t>> * groupsRes = nullptr) = delete;
};
...
template<class DTRes>
void order(DTRes *& res, const DTRes * arg, size_t * colIdxs, size_t numKeyCols, bool * ascending, DCTX(ctx), std::vector<std::pair<size_t, size_t>> * groupsRes = nullptr) {
    Order<DTRes>::apply(res, arg, colIdxs, numKeyCols, ascending, ctx, groupsRes);
}

template<class DTRes>
void orderGroups(DTRes *& res, const DTRes * arg, size_t * colIdxs, size_t numKeyCols, bool * ascending, DCTX(ctx), std::vector<std::pair<size_t, size_t>> * groupsRes = nullptr) {
    Order<DTRes>::apply(res, arg, colIdxs, numKeyCols, ascending, ctx, groupsRes);
}
...
template <> struct Order<Frame> {
    static void apply(Frame *& res, const Frame * arg, size_t * colIdxs, size_t numKeyCols, bool * ascending, DCTX(ctx), std::vector<std::pair<size_t, size_t>> * groupsRes = nullptr) {
        ...
        size_t colIdx = colIdxs[numKeyCols-1];
        if (groupsRes == nullptr) {
            DeduceValueTypeAndExecute<ColumnIDSort>::apply(arg->getSchema()[colIdx], arg, idx, groups, ascending[numKeyCols-1], colIdx, ctx);
        } else {
            DeduceValueTypeAndExecute<MultiColumnIDSort>::apply(arg->getSchema()[colIdx], arg, idx, groups, ascending[numKeyCols-1], colIdx, ctx);
