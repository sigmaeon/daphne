...
function cleanAll() {
    ...
    local dirs=("${buildPrefix}" "${sourcePrefixPrefix}" "${installPrefixPrefix}")
    local files=(\
      "${thirdpartyPath}/antlr_v"*".install.success" \
...
installPrefix="${thirdpartyPath}/installed"
buildPrefix="${thirdpartyPath}/build"
sourcePrefix="${thirdpartyPath}/sources"
cacheDir="${thirdpartyPath}/download-cache"
...
if [ -d .git ] # Note: .git in the submodule is not a directory.
then
...
cmake -S "$projectRoot" -B "$daphneBuildDir" -G Ninja -DCMAKE_PREFIX_PATH="$installPrefix" \
  -DANTLR_VERSION="$antlrVersion" $BUILD_CUDA $BUILD_DEBUG
